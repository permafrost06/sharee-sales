...
@section('page')
    ...
    <x-cards.card class="max-w-4xl">
        <div class="flex items-center py-2 border-b px-6">
            <h3 class="flex-grow text-dark">{{ $sale ? 'Update' : 'Add New' }} Deposit</h3>
            <a href="{{ route('sales.index') }}" class="bg-blue-500 text-white px-4 py-2 rounded-md">
                View All
            </a>
        </div>
        <form action="{{ route('sales.store', ['id' => $sale?->id]) }}" method="POST" class="p-6">
            ...
            <div class="space-y-4">
                ...
                <x-form.input type="number" min="0" step="0.01" label="Received Money" name="received_money" :value="old('received_money', $sale?->received_money)"/>
                <x-form.input type="number" min="0" step="0.01" label="Balance (DUE)" name="balance_due" :value="old('balance_due', $sale?->balance_due)"/>
