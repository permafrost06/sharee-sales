                                ...
                                @if(isset($customerDeposits))
                                    @foreach($customerDeposits as $k=>$customerDeposit)
                                        <tr>
                                            ...
                                            <td> {{ ($customerDeposit->goods_of_issues - $customerDeposit->received_money) }}</td>
                                            <td> {{ $customerDeposit->comment }}</td>
                                            <td><input type="checkbox" onclick="checkedItem({{$customerDeposit->id}})"></td>

                                            {{--<td>{{ date('y-m-d',strtotime($customerDeposit->created_at)) }}</td>--}}
                                            <td>
                                                <a class="btn btn-xs btn-primary" href="{{route('sales.edit',['id'=>$customerDeposit->id])}}"><i class="fa fa-edit"></i></a>
                                                <a class="btn btn-xs btn-danger" href="{{route('sales.delete',['id'=>$customerDeposit->id])}}"><i class="fa fa-trash"></i></a>
...
{{--                    </form>--}}
                    @if(count($customerDeposits)>0)
                        <a href="{{route('sales.generatePDF',['id'=>$customerDeposits[0]->customer_id])}}"> Download pdf</a>
                    @endif
