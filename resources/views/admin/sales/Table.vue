...
const isGlobal = !SALES_API_LINK.match(/[0-9]$/);
console.log(isGlobal, SALES_API_LINK);
...
const editLink = (item) => SALE_EDIT_LINK.replace('::ID::', item.id);
...
const showDelete = (item) => {
    deleteUrl.value = SALE_DELETE_LINK.replace('::ID::', item.id);
    toDelete.value = item;
}
...
<template>
            ...
            <thead class="text-xs uppercase bg-skin-neutral bg-opacity-5">
                <tr>
                    ...
                    <th
                        ...
                        class="px-6 py-3 sortable"
                        :class="{
                            [sorted.order]: sorted.by === 'received_money',
                        }"
                        @click="() => sortBy('received_money')"
                    >
                        Received Money
                    </th>
    ...
    >
        ...
        Are you sure you want to delete the sale <b>{{ toDelete.name }}</b>?
    </DeleteModal>
