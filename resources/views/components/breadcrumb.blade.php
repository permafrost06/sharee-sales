<nav class="flex" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-1 md:space-x-3">
        <li class="inline-flex items-center">
            <a href="{{ $home['link'] }}" class="inline-flex items-center font-medium hover:text-active">
        ...
        @foreach ($items as $item)
            <li>
                <div class="flex items-center">
                    ...
                    @if (isset($item['link']))
                        <a href="{{ $item['link'] }}"
                            class="ml-1 font-medium hover:text-active md:ml-2">{{ $item['label'] }}</a>
                    @else
                        {{ $item['label'] }}
                    @endif
        ...
        <li aria-current="page">
            <div class="flex items-center">
                ...
                <span class="ml-1 font-medium text-gray-400 md:ml-2">{{ $active }}</span>
            </div>
        </li>
        @endif
    </ol>
