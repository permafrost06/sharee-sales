            ...
            <ul class="flex items-center space-x-3">
                <li>
                    <div class="px-4 py-3 flex items-center gap-1" title="{{ auth()->user()->email }}">
                        ...
                        <h1 class="text-base font-bold text-gray-900">{{ auth()->user()->name }}</h1>
                    </div>
                </li>
                <li>
                    <form action="{{ route('logout') }}" method="POST" class="block">
                        @csrf
                        <button class="flex gap-2 items-center px-4 py-2 rounded text-sm font-bold uppercase text-white bg-blue-500 hover:bg-blue-600 w-full text-left"
                            type="submit">
